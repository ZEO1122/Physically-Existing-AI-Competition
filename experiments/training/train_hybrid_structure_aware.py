...
def autocast_context(device_type: str, enabled: bool):
    """
    AMP context를 안전하게 여는 헬퍼입니다.
    - CUDA + AMP enabled일 때만 torch.amp.autocast("cuda")를 사용합니다.
    - 그 외에는 그냥 일반 precision으로 실행합니다.
    - 내부 forward에서 발생한 예외를 삼키지 않도록 단순 구조로 유지합니다.
    """
...
def extract_object_mask_from_pil(img: Image.Image, view: str) -> Image.Image:
    """
    구조물은 배경 checkerboard / sky보다 채도와 colorfulness가 높고,
    블록 경계 edge가 많다는 점을 이용해 heuristic mask를 만듭니다.
    """
    ...
    # glare / sky 영역은 매우 밝고 채도가 낮은 경우가 많아서 억제합니다.
...
def compute_geometry_features(
    ...
    if len(features) != GEOM_FEATURE_DIM:
        raise ValueError(
            f"Geometry feature dim mismatch: expected {GEOM_FEATURE_DIM}, got {len(features)}"
        )
...
class StructureAwarePairTrainTransform:
    ...
    photometric 증강은 RGB에만 적용합니다.
    """
    def __init__(self, img_size: int, grid_xy: int, grid_z: int):
    ...
    def __call__(self, front: Image.Image, top: Image.Image, front_mask: Image.Image, top_mask: Image.Image):
        ...
        if random.random() < 0.08:
            buf = io.BytesIO()
            front.save(buf, format="JPEG", quality=random.randint(48, 82))
            buf.seek(0)
            front = Image.open(buf).convert("RGB")
            buf = io.BytesIO()
            top.save(buf, format="JPEG", quality=random.randint(48, 82))
            buf.seek(0)
        ...
        return front_t, top_t, front_mask_t, top_mask_t, geom
...
class StructureAwareDataset(Dataset):
    def __init__(self, df: pd.DataFrame, transform=None, is_test: bool = False, cache_masks: bool = True):
    ...
    def _get_masks(self, sample_id: str, front: Image.Image, top: Image.Image) -> Tuple[Image.Image, Image.Image]:
        if self.cache_masks and sample_id in self.mask_cache:
            fm, tm = self.mask_cache[sample_id]
            return fm.copy(), tm.copy()

        front_mask = extract_object_mask_from_pil(front, view="front")
        top_mask = extract_object_mask_from_pil(top, view="top")

        if self.cache_masks:
            self.mask_cache[sample_id] = (front_mask.copy(), top_mask.copy())
        return front_mask, top_mask
    ...
    def __getitem__(self, idx: int):
        ...
        front_mask, top_mask = self._get_masks(sample_id, front, top)

        if self.transform is not None:
            front, top, front_mask, top_mask, geom = self.transform(front, top, front_mask, top_mask)
        else:
            raise RuntimeError("transform is required for StructureAwareDataset")

        if self.is_test:
            return front, top, front_mask, top_mask, geom, sample_id

        label = torch.tensor([float(row["label_float"])], dtype=torch.float32)
        return front, top, front_mask, top_mask, geom, label, sample_id
...
class StructureAwareFusionNet(nn.Module):
    ...
    또한 RGB 융합 특징으로 geometry를 예측하는 auxiliary head를 둡니다.
    """
    def __init__(self, geom_dim: int, pretrained: bool = True, dropout: float = 0.30):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(rgb_fuse_dim + 128 + 128, 512),
            nn.LayerNorm(512),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(512, 128),
    ...
    def forward(self, front: torch.Tensor, top: torch.Tensor, front_mask: torch.Tensor, top_mask: torch.Tensor, geom: torch.Tensor):
        ...
        logits = self.classifier(torch.cat([rgb_fused, mask_feat, geom_feat], dim=1))
        aux_geom = self.aux_geom_head(rgb_pair)
        return {"logits": logits, "aux_geom": aux_geom}
...
@dataclass
class Config:
    run_name: str = "hybrid_structure_aware"
    mode: str = "holdout"
    data_root: str = "./open"
    save_dir: str = "./runs_hybrid_structure_aware"
...
def build_loader(df: pd.DataFrame, transform, cfg: Config, device: torch.device, is_test: bool, shuffle: bool, sampler=None) -> DataLoader:
    ds = StructureAwareDataset(df, transform=transform, is_test=is_test, cache_masks=True)
...
def build_optimizer_and_scheduler(model: StructureAwareFusionNet, cfg: Config, steps_per_epoch: int):
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=[cfg.backbone_lr, cfg.learning_rate, cfg.learning_rate, cfg.learning_rate, cfg.learning_rate, cfg.learning_rate, cfg.learning_rate, cfg.learning_rate],
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, cfg: Config, device: torch.device, use_amp: bool) -> float:
    ...
    for front, top, front_mask, top_mask, geom, y, _ in tqdm(loader, leave=False):
...
def validate_one_epoch(model, loader, cfg: Config, device: torch.device, use_amp: bool) -> EpochOutput:
    ...
    for front, top, front_mask, top_mask, geom, y, sample_ids in loader:
        front = front.to(device, non_blocking=True)
        top = top.to(device, non_blocking=True)
        front_mask = front_mask.to(device, non_blocking=True)
        top_mask = top_mask.to(device, non_blocking=True)
        ...
        with autocast_context(device.type, use_amp):
            pred = model(front, top, front_mask, top_mask, geom)["logits"]
...
def infer_logits(model, loader, device: torch.device, use_amp: bool, tta_hflip: bool) -> Tuple[np.ndarray, List[str]]:
    ...
    for front, top, front_mask, top_mask, geom, sample_ids in tqdm(loader, leave=False):
        front = front.to(device, non_blocking=True)
        top = top.to(device, non_blocking=True)
        front_mask = front_mask.to(device, non_blocking=True)
        top_mask = top_mask.to(device, non_blocking=True)
        ...
        with autocast_context(device.type, use_amp):
            logits = model(front, top, front_mask, top_mask, geom)["logits"]
            if tta_hflip:
                logits_flip = model(
...
def train_with_validation(
    ...
    train_tf = StructureAwarePairTrainTransform(cfg.img_size, cfg.grid_xy, cfg.grid_z)
    eval_tf = StructureAwarePairEvalTransform(cfg.img_size, cfg.grid_xy, cfg.grid_z)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hybrid structure-aware trainer")
    parser.add_argument("--mode", choices=["holdout", "cv"], default="holdout")
    parser.add_argument("--data_root", type=str, default="./open")
    parser.add_argument("--save_dir", type=str, default="./runs_hybrid_structure_aware")
    parser.add_argument("--run_name", type=str, default="hybrid_structure_aware")
